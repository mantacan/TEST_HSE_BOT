...
async def test_update_location_command():
    ...
    message = Message(
        message_id=1, from_user=user, chat=chat, date=None, text="/update_location"
    )
...
async def test_update_location_success():
    ...
    message = Message(
        message_id=1, from_user=user, chat=chat, date=None, text=None, location=location
    )
    update = Update(update_id=1, message=message)
    context = Mock(spec=CallbackContext)
    ...
    with patch.object(Message, "reply_text", new=AsyncMock()) as mock_reply:
        ...
        db_manager.update_user_location.assert_called_once_with(
            "@testuser", 55.7558, 37.6173
        )
...
async def test_show_current_location_success():
    ...
    chat = Chat(id=12345, type="private")
    message = Message(
        message_id=1, from_user=user, chat=chat, date=None, text="/show_location"
    )
    update = Update(update_id=1, message=message)
    context = Mock(spec=CallbackContext)
