...
class Controller:
    ...
    def _register_handlers(self):
        ...
        self.application.add_handler(CommandHandler("status_group", self.group.show_group_info))
        self.application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, self.greet))
        self.application.add_handler(CallbackQueryHandler(self.button_handler))
        self.application.add_handler(CommandHandler("search_for_group", self.search.search_for_group))
        self.application.add_handler(CommandHandler("current_location", self.location_manager.show_current_location))
        self.application.add_handler(CommandHandler("update_location", self.location_manager.update_location_command))
        self.application.add_handler(MessageHandler(filters.LOCATION, self.location_manager.update_location))
        self.application.add_handler(CommandHandler("search_nearby", self.search_nearby))
        self.application.add_handler(CommandHandler("add_to_group", self.group.add_to_group))
        self.application.add_handler(CommandHandler("show_group_info", self.group.show_group_info))
        self.application.add_handler(CommandHandler("remove_from_group", self.group.remove_from_group))
    ...
    async def search_nearby(self, update: Update, context: CallbackContext) -> None:
        ...
        if location and location[0] is not None and location[1] is not None:
            ...
            error_message = "Сначала обновите свою геолокацию с помощью команды /update_location."
    ...
    async def start(self, update: Update, context: CallbackContext) -> None:
        ...
        keyboard = [
            [KeyboardButton("/menu"), KeyboardButton("Обновить геолокацию", request_location=True)]
        ]
        ...
        inline_keyboard = [
            ...
            [InlineKeyboardButton("Найти бар для группы", callback_data='search_for_group')]
        ]
        ...
        await update.message.reply_text(welcome_message, reply_markup=inline_reply_markup)
        await update.message.reply_text('Меню доступно в любое время.', reply_markup=reply_markup)
    ...
    async def menu_handler(self, update: Update, context: CallbackContext) -> None:
        keyboard = [
            ...
            [InlineKeyboardButton("Обновить геолокацию", callback_data='update_location')],
            [InlineKeyboardButton("Текущая геолокация", callback_data='current_location')],
            [InlineKeyboardButton("Показать список группы", callback_data='show_group_info')],
            [InlineKeyboardButton("Найти бар для группы", callback_data='search_for_group')]
        ]
    ...
    async def info(self, update: Update, context: CallbackContext, is_callback=False) -> None:
        info_message = (
            "Спасибо за использование данного бота, с его помощью вы сможете найти ближайший бар для вас и ваших друзей, бот работает "
            "в двух режимах:\n"
            "Одиночный режим: бот ищет бар/клуб исходя из вашей геолокации\n"
